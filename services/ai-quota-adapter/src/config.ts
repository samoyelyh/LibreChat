...
const envSchema = z.object({
  ...
  NEW_API_REQUEST_TIMEOUT_MS: z.coerce.number().int().min(1000).max(600000).default(300000),
  AI_ADAPTER_BODY_LIMIT_BYTES: z.coerce
    .number()
    .int()
    .min(1024 * 1024)
    .max(32 * 1024 * 1024)
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AdapterConfig {
  ...
  return {
    ...
    bodyLimitBytes: parsed.AI_ADAPTER_BODY_LIMIT_BYTES,
