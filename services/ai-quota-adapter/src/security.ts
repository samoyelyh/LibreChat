...
export function verifySignedRequest(
  ...
  const expectedHash = createHash('sha256').update(bodyText(request.body)).digest('hex');
...
export function createSignedHeaders(input: {
  ...
  let canonicalBody = input.bodyText ?? '';
  try {
    canonicalBody = input.bodyText ? canonicalJSON(JSON.parse(input.bodyText) as JsonValue) : '';
  } catch {
    canonicalBody = input.bodyText ?? '';
  }
  const contentHash = createHash('sha256').update(canonicalBody).digest('hex');
  const value = (name: string): string =>
    Object.entries(input.actorHeaders).find(([key]) => key.toLowerCase() === name)?.[1] ?? '';
  const actor = [
    'x-librechat-user-id',
    'x-librechat-user-email',
    'x-librechat-user-role',
