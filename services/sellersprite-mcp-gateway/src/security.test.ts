...
function signedHeaders(body: string, timestamp = now): Record<string, string> {
...
describe('signed gateway requests', () => {
  it('accepts a canonical signed request regardless of JSON key order', async () => {
    expect(await verify('{"b":2,"a":1}', signedHeaders('{"b":2,"a":1}'))).toEqual({
      ok: true,
      nonce,
  ...
  it('rejects changed bodies and stale timestamps', async () => {
    expect(await verify('{"a":9}', signedHeaders('{"a":1,"b":2}'))).toEqual({
      ok: false,
      code: 'invalid_request_signature',
    });
    expect(await verify('{"a":1,"b":2}', signedHeaders('{"a":1,"b":2}', now - 31_000))).toEqual({
      ok: false,
