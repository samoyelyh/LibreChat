...
- 只有存在 `active` AI Gateway 映射的 LibreChat 用户才能访问公司模型；当前唯一映射属于系统 `ADMIN`，原 Phase 2 测试用户已取消映射。
- 用户只能看到映射中的模型白名单；2026-07-24 已从 New API 显式同步 29 个模型，伪造或未同步的模型 ID 会在 Adapter 后端返回 403。
...
## Phase 5 共享 Agent 与私有 Skill 权限补充

本节覆盖上方 Phase 1 基线中的 Agent/Skill 状态；MCP 权限仍分别遵循 Phase 3 和 Phase 4 矩阵。

| LibreChat Role          | Agent            | Skill            | 创建/编辑/共享   | 读取 Skill 正文与附件 |
| ----------------------- | ---------------- | ---------------- | ---------------- | --------------------- |
| 内置 `ADMIN`            | 使用             | 使用             | 是，禁止公开共享 | 是                    |
| `USER` / 六个自定义角色 | 使用已授权 Agent | 使用已授权 Skill | 否               | 否                    |

业务 Skill 使用 `executionOnly: true` 和 Viewer ACL。普通用户只能读取安全摘要，详情正文被清空，frontmatter、来源元数据和附件接口被拒绝。当前测试用户仍无 New API 映射；看到 Agent 不等于可以借用管理员 Token。
