...
tag="woda/librechat:v0.8.7-phase7-structured"
docker build \
  --pull=false \
  --file "$ROOT_DIR/Dockerfile.multi" \
  --tag "$tag" \
  --build-arg BUILD_COMMIT="$expected_commit" \
  --build-arg BUILD_BRANCH="woda/phase-7" \
...
docker run --rm --entrypoint sh "$image_id" -c \
  ...
  printf 'Structured ecommerce result UI is missing from the built client.\n' >&2
  exit 1
}
...
chmod 0600 "$ENV_FILE"
printf 'LibreChat Phase 7 structured-result image built from v0.8.7 and pinned by local image ID.\n'
