...
tag="woda/librechat:v0.8.7-phase5-quota"
docker build \
  --pull=false \
  --file "$ROOT_DIR/Dockerfile.multi" \
  --tag "$tag" \
  --build-arg BUILD_COMMIT="$expected_commit" \
  --build-arg BUILD_BRANCH="woda/phase-4" \
...
chmod 0600 "$ENV_FILE"
printf 'LibreChat Phase 5 quota image built from v0.8.7 and pinned by local image ID.\n'
